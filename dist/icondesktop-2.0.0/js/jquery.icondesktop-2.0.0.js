...
;(function (factory) {
  ...
  $.fn.icondesktop = function (options) {
    var defaultOption = {
      ...
      desktopAutoFill: true, // 组合/移动图标出现空缺后，桌面是否自动填满
      pageHeight: 50, // 分页栏高度
    ...
    this.each(function () {
      ...
      $(this).off('mousedown', '.icondesktopbox').on('mousedown', '.icondesktopbox', function (e) {
        ...
        e.preventDefault();
        e.stopPropagation();
        // 是桌面元素非打开状态按下或者是打开盒子的盒子内元素，并且可以拖动
        if (!$this.hasClass('iconbox__open') && opt.ableDrag) {
          dragDown($this, opt, e);
        }
      });
      // 鼠标按下盒子里的元素事件
      $(this).off('mousedown', '.iconinbox').on('mousedown', '.iconinbox', function (e) {
        var $this = $(this);
        opt.moveObj = $this;
        var $pBox = $this.parents('.icondesktopbox');
        e.preventDefault();
        if ($pBox.hasClass('iconbox__close')) {
          // 如果是关闭的盒子里的元素(上方条件)，则不做什么
          return;
        }
        e.stopPropagation();
        // 是打开盒子的盒子内元素，并且可以拖动
        if ($pBox.hasClass('iconbox__open') && opt.ableDrag) {
          dragDown($this, opt, e);
        }
      });
      // 鼠标离开桌面恢复
  ...
  function recoverLocationInBox(dataBox, opt, isAnimate) {
    ...
    if (isAnimate) {
      ...
      for (var i = 0; i < dataBox.children.length; i++) {
        var element = dataBox.children[i];
        var point = getLocationInCloseBox(i, opt);
        // 超过关闭盒子最多显示数量时，下移一定距离，完全移出就看不见了
        if (i > opt.maxShowIconInBox) {
          point.top += opt.thumbnailHeight;
        }
        element.$dom.animate({
          'left': point.left + 'px',
          'top': point.top + 'px',
  ...
  function dragDown($this, opt, e) {
    ...
    opt.moveIconObj.timeout = setTimeout(function () {
      ...
      //   var scrollTop = $this.parents('.iconbox-area__container').scrollTop();
      //   // 如果此时是在盒子里(上方条件)，则需要将盒子里的位置换算成桌面上的位置
      //   iconLeft += opt.currentPageIndex * opt.desktopWidth + opt.openBoxMarginLeft;
      //   iconTop += opt.openBoxMarginTop - scrollTop;
      // }
      var desktopPoint = switchToDesktopPoint($this, iconLeft, iconTop, opt);
      iconLeft = desktopPoint.left;
      iconTop = desktopPoint.top;
      // 指示位置图标透明度
      ...
      if (!moveIconObj.iconInterval) {
        ...
        moveIconObj.iconInterval = setInterval(function () {
          ...
          if ($flagDom.hasClass('icondesktopbox')) {
            ...
            // 在盒子里
            // getIconData($this, opt).size
            moveLocation = getMoveLocation($flagDom, '1x1', opt.currentPageIndex, opt.desktopWidth, opt.separationBoxLine, opt.verIconSize, opt.horIconSize);
          }
    ...
    if ($this.attr('isMouseDownIcon')) {
      ...
      if ($this.attr('isMouseDownMove')) {
        ...
        if (moveIconObj.$prevIconBelow === $iconBelow) {
          ...
          // 上方有操作节点的操作，会有一定的耗时，如果不用定时器，看上去操作的盒子动画会慢上半拍
          setTimeout(function () {
            refreshDoms(null, opt, opt.desktopAutoFill);
          }, 300);
          moveFlag = 2;
        } else {
          ...
          // }
          var desktopPoint = switchToDesktopPoint($this, theLeft, theTop, opt);
          theLeft = desktopPoint.left;
          theTop = desktopPoint.top;
          if ($this.hasClass('iconbox-a')) {
            // 如果是图标(上方条件)，则调整图标大小
  ...
  function getIconIndex($icon, opt) {
    ...
    var iconIndex = getIndexByLocation(location, opt.data, opt.locationObj);
    return iconIndex;
  }
  ...
  function groupData(data, srcIndex, dstIndex, opt) {
    ...
    var newData;
    // 删除占用位置信息
    delete opt.locationObj[srcData.location];
    if (dstData.img) {
      // 目标是个图标
      newData = {
        title: '新分组',
      ...
      // 加上位置信息
      var locationIconObj = {};
      opt.locationBoxObj[newData.location] = locationIconObj;
      dstData.location = getFirstEmptyLocationInBox(locationIconObj, opt);
      srcData.location = getFirstEmptyLocationInBox(locationIconObj, opt);
      ...
      newData = dstData;
      // 加上位置信息
      var locationIconObj = opt.locationBoxObj[newData.location];
      srcData.location = getFirstEmptyLocationInBox(locationIconObj, opt);
    }
    return newData;
  }
  ...
  function groupDoms($this, $iconBelow, opt, newData) {
    if ($iconBelow.hasClass('iconbox-a')) {
      ...
      $this.css({
        ...
        'cursor': '',
        'zIndex': ''
      }).removeClass('icondesktopbox').addClass('iconinbox');
      $this.find('.iconbox-checkbox').hide();
      $this.find('.iconbox-delbtn').hide();
      // 重新绑定多选框点击事件
      bindCheckboxClick($this.children('.iconbox-checkbox'), newData.children[newData.children.length - 1], newData);
      $this.appendTo($iconBelow.find('.iconbox-area__container')).show();
      // 判断是否已经超过9个图标
      // if ($this.index() > opt.maxShowIconInBox) {
      //   $this.hide();
      // } else {
  ...
  function addIcons($root, opt, data, isOnDesktop) {
    if (isOnDesktop) {
      ...
      if (opt.openBox.$box) {
        ...
        // 添加位置信息
        data.location = getFirstEmptyLocationInBox(opt.locationBoxObj[boxData.location], opt);
        // 插入数据
        insertData(boxData.children, data);
        // 排序
        boxData.children.sort(sortByLocation);
        // 插入节点
        ...
        data.location = getFirstEmptyLocation(data.size, opt);
        // 插入数据
        insertData(opt.data, data);
        // 排序
        opt.data.sort(sortByLocation);
        // 插入节点
        var icons = insertIcons($root, opt, data, null);
        /**
         * 调整页数
         */
        var locationInfo = parseLocationInfo(data.location);
        var pages = locationInfo.page + 1;
        if (pages > opt.pages) {
          adjustPageBox(icons[0], opt.pages, pages);
          opt.pages = pages;
        }
        // var $icon = constructIcon(opt, null, data);
        // $root.find('.icondesktop-slidebox').append($icon);
        // refreshDesktop($root, opt);
      }
    }
  }
  ...
  function insertIcon($box, opt, dataIcon, dataBox) {
    ...
    setLocations($icon, dataIcon, opt, 'set');
    if ($box.hasClass('iconbox')) {
      $box.find('.iconbox-area__container').append($icon);
    } else {
      $box.find('.icondesktop-slidebox').append($icon);
    }
    return $icon;
  }
  ...
  function insertIcons($box, opt, data, dataBox) {
    ...
    if (data.push) {
      ...
      for (var i = 0; i < data.length; i++) {
        var element = data[i];
        var $icon = insertIcon($box, opt, element, dataBox);
        icons.push($icon);
      }
    } else {
      // 如果data是单一对象，则直接添加
      var $icon = insertIcon($box, opt, data, dataBox);
      icons.push($icon);
    }
    return icons;
  }
  ...
  function separateDesktopGrids($root, opt) {
    ...
    // 填充数据的位置信息，以及总页数
    fillLocation(opt.data, opt, true);
  ...
  function separateBoxGrids($root, opt) {
    ...
    var verIconSize = 100; // 盒子内固定最多100行
    ...
    for (var i = 0; i < opt.data.length; i++) {
      var element = opt.data[i];
      var locationIconObj = {};
      opt.locationBoxObj[element.location] = locationIconObj;
      if (element.children && element.children.length) {
        fillLocationInBox(element.children, locationIconObj, opt, true);
      }
    }
    ...
    var halfBoxWidth = opt.closeBoxWidth / 2;
    var halfBoxHeight = opt.closeBoxHeight / 2;
    var maxLineY = opt.desktopHeight - opt.openBoxMarginBottom;
    // 循环行
    // var i = 0;
    // do {
  ...
  /**
   * [fillLocation 向data中填充location，如果是覆盖，则还要改变对应的locationBoxObj]
   * @param  {[type]} data     [description]
   * @param  {[type]} opt      [description]
   * @param  {[type]} override [是否覆盖原来的location]
   * @param  {[type]} exclusion [description]
   * @return {[type]}           [description]
   */
  function fillLocationxx(data, opt, override, exclusionNewLocation, exclusionOldLocation) {
    var locationObj = opt.locationObj;
    if (override) {
      clearObj(locationObj, exclusionNewLocation);
    }
    var pageIndex = 0;
    var filledLocations = [];
    var changedLocationRelative = {};

    // debugger
    for (var i = 0; i < data.length; i++) {
      var element = data[i];
      var elementSize = element.size;
      if (override || !element.location) {
        // 如果是覆盖位置信息或者位置信息不存在时(上方条件)
        var prevLocation = element.location;
        if (exclusionNewLocation && exclusionNewLocation === prevLocation && element.$dom.attr('location') != prevLocation) {
          // 如果有排除的位置，并且排除的位置就是目前这个element的位置(上方条件)，则不更新位置信息
          changedLocationRelative[exclusionOldLocation] = exclusionNewLocation;
        } else {
          // 不是排除的位置(上方条件)，则补上位置信息
          element.location = getFirstEmptyLocation(elementSize, opt);
          // 对于覆盖的location，记录下来改变的location
          if (prevLocation) {
            changedLocationRelative[prevLocation] = element.location;
          }
        }
    ...
    /**
     * [if 覆盖模式下，还需要调整对应的盒子图标信息]
     */
    if (override) {
      var locationBoxObj = opt.locationBoxObj;
      var tempLocationBoxObj = {};
      for (var key in changedLocationRelative) {
        var value = changedLocationRelative[key];
        tempLocationBoxObj[value] = locationBoxObj[key];
        delete locationBoxObj[key];
      }
      for (var key in tempLocationBoxObj) {
        locationBoxObj[key] = tempLocationBoxObj[key];
      }
    }
  ...
  function fillLocation(data, opt, override, locations) {
    var locationObj = opt.locationObj;
    if (override) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var elementSize = element.size;
      var prevLocation = element.location;
      if (override || !prevLocation) {
        // 如果是覆盖位置信息或者位置信息不存在时(上方条件)
        // 不是排除的位置(上方条件)，则补上位置信息
        element.location = getFirstEmptyLocation(elementSize, opt);
        // 对于覆盖的location，记录下来改变的location
        if (prevLocation) { // 有位置信息
          changedLocationRelative[prevLocation] = element.location;
        } else { // 没有位置信息
          if (locations && locationIndex < locations.length) {
            changedLocationRelative[locations[locationIndex++]] = element.location;
          }
        }
        filledLocations.push(element.location);
        // 如果数据关联了dom节点，则修改dom节点的位置信息记录
    ...
    /**
     * [覆盖模式下，还需要调整对应的盒子图标信息]
     */
    // if (override) {
      var locationBoxObj = opt.locationBoxObj;
      var tempLocationBoxObj = {};
      for (var key in changedLocationRelative) { // 循环原位置
        var value = changedLocationRelative[key]; // 新位置
        // 原位置的对应盒子内图标信息 -> 新位置
        tempLocationBoxObj[value] = locationBoxObj[key];
        // 删除原位置的对应盒子内图标信息
        delete locationBoxObj[key];
  ...
  /**
   * [fillLocationInBox 填充盒子里图标的位置]
   * @param  {[type]} children        [description]
   * @param  {[type]} locationIconObj [description]
   * @param  {[type]} opt             [description]
   * @param  {[type]} override        [是否覆盖原来的位置]
   * @return {[type]}                 [description]
   */
  function fillLocationInBox(children, locationIconObj, opt, override) {
    if (override) {
      clearObj(locationIconObj);
    }
    for (var i = 0; i < children.length; i++) {
      var element = children[i];
      if (override || !element.location) {
        // 如果位置信息不存在补上位置信息
        element.location = getFirstEmptyLocationInBox(locationIconObj, opt);
      }
      // 如果数据关联了dom节点，则修改dom节点的位置信息记录
      if (element.$dom) {
        setLocationAttr(element.$dom, element.location);
  ...
  function getFirstEmptyLocationInBox(locationIconObj, opt) {
    var row = 0;
    var col = 0;
    var curLocation;
    outer:
   ...
   * @param  {[type]} scrollTop        [盒子内滚动的高度]
   * @return {[type]}                  [description]
   */
  function getMoveLocation($dom, size, currentPageIndex, desktopWidth, separationLine, rowSize, colSize, scrollTop) {
    // 当前页数
   ...
   * @param  {[type]} $dom    [移动的节点]
   * @param  {[type]} domData [对应的节点数据]
   * @param  {[type]} boxData [盒子的数据]
   * @param  {[type]} opt     [description]
   * @return {[type]}         [description]
   */
  function moveToLocationInBox($dom, domData, boxData, opt) {
    var children = boxData.children;
    var scrollTop = boxData.$dom.find('.iconbox-area__container').scrollTop();
    var moveLocation = getMoveLocation($dom, domData.size, opt.currentPageIndex, opt.desktopWidth, opt.separationBoxLine, opt.verIconSize, opt.horIconSize, scrollTop);
    // 微调moveLocation中location的值
    adjustMoveLocationInBox(moveLocation);

    var pos = moveLocation.pos;
    if (pos === 'left' || pos === 'right' || pos === 'top' || pos === 'bottom') {
      // 如果移出盒子(上方条件)
      // debugger
      moveIconToDesktopFromBox($dom, domData, boxData, opt);
    } else if (pos === 'over') {
      // 如果放不下(上方条件)，则不作任何处理
      ...
      if (domData.location === moveLocation.location) {
        ...
        if (moveLocation.isEnter) {
          // 如果是移入状态(上方条件)，则判断移入位置是图标、盒子、还是工具
          var theData = getDataByLocation(moveLocation.location, children, opt.locationBoxObj[boxData.location]);
          if (theData == null) {
            // 如果该位置为空(上方条件)，则直接移动过去
            moveToolInBox(opt.flagObj, domData, children, moveLocation, opt.locationBoxObj[boxData.location], opt);
          } else if (theData.img) {
            // 如果是图标(上方条件)，则移动过去
            console.log(domData.location)
            moveIconInBox(opt.flagObj, domData, children, moveLocation.location, opt.locationBoxObj[boxData.location], opt);
          } else if (theData.type === 'tool') {
            // 如果是工具(上方条件)，目前不可能
  ...
  function moveIconOrBox($flagDom, domData, data, moveLocation, locationObj, opt) {
    cancelGroupAndJoin(opt);
    var oldLocation = domData.location;
    var newLocation = moveLocation.location;
    var locations = [];
    clearObj(locationObj);
    locationObj[newLocation] = createLocationRelativeInfo(newLocation, domData.size);
    // 此时桌面上只剩一个占有位置
    for (var i = 0; i < data.length; i++) {
      var element = data[i];
      if (element !== domData && element.location) { // 不是移动的数据并且有位置信息
        locations.push(element.location); // 位置信息放入locations中用于后面更新locationBoxObj
        delete element.location; // 清除位置信息
      }
    }
    domData.location = newLocation;
    // 此时data里只有移动的数据有位置信息
    
    var locationIconObj = opt.locationBoxObj[oldLocation]; // 1
    delete opt.locationBoxObj[oldLocation]; // 2

    var pages = opt.pages;
    fillLocation(data, opt, false, locations);
    // fillLocation(data, opt, false, moveLocation.location, oldLocation);
    opt.locationBoxObj[newLocation] = locationIconObj; // 3
    ...
    animateDoms(data, opt, getIndexByLocation(moveLocation.location, data, opt.locationObj));
    // refreshDoms();
  }
  ...
  function moveTool($flagDom, domData, data, moveLocation, locationObj, opt) {
    ...
    var pages = opt.pages;
    // 填充删除的位置信息与占用信息
    var filledLocations = fillLocation(data, opt, false, locations);
    // var filledLocations = fillLocation(data, opt);
    opt.locationBoxObj[newLocation] = locationIconObj; // 3
  ...
  function moveIconInBox($flagDom, domData, children, location, locationIconObj, opt) {
    clearObj(locationIconObj);
    locationIconObj[location] = createLocationRelativeInfo(location, domData.size);
    clearDataLocation(children);
    domData.location = location;
    setLocationAttr($flagDom, domData.location);
    setLocationAttr(domData.$dom, domData.location);
    
    fillLocationInBox(children, locationIconObj, opt);
    console.log(location)
    
    console.log(getIndexByLocation(location, children, locationIconObj));
    animateDoms(children, opt, getIndexByLocation(location, children, locationIconObj));
  }

  function moveToolInBox($flagDom, domData, children, moveLocation, locationObj, opt) {
    // 清除工具原来位置的占用信息
    var occupiedLocations = getOccupiedLocations(domData.location, domData.size);
    for (var i = 0; i < occupiedLocations.length; i++) {
      var element = occupiedLocations[i];
    ...
    for (var i = 0; i < moveLocation.locations.length; i++) {
      var element = moveLocation.locations[i];
      // 查询这些位置对应的data中的数据
      var dData = getDataByLocation(element, children, locationObj);
      if (dData) {
        // 删除这些data数据对应的location信息
        delete dData.location;
      }
    ...
    // 填充删除的位置信息与占用信息
    fillLocationInBox(children, opt);
    animateDoms(children, opt, getIndexByLocation(moveLocation.location, children, opt.locationObj));
  }
  ...
  //   var $dom = domData.$dom;
  //   var moveIndex = getIndexByLocation(moveLocation.location, data, opt.locationObj);
  ...
  function getMoveIndexAndRelativeIndex(domData, data, moveLocation, opt) {
    var locationObj = opt.locationObj;
    // 移动物体关联的数据在data中的序数
    var moveIndex = getIndexByLocation(domData.location, data, opt.locationObj);
    ...
    if (!locationObj[moveLocation.location]
      ...
      if (nextLocation == null) {
        ...
        relativeIndex = getIndexByLocation(nextLocation, data, opt.locationObj);
      }
    } else {
      relativeIndex = getIndexByLocation(moveLocation.location, data, opt.locationObj);
    }
    return {
      moveIndex: moveIndex,
   ...
   * @return {[type]}      [description]
   */
  function animateDoms(data, opt, excludeIndex, callback) {
    var total = 0;
    for (var i = 0; i < data.length; i++) {
      var element = data[i];
      var $dom = element.$dom;
      ...
      if (i === excludeIndex) {
        ...
        animateToLocation($dom, location, opt, function () {
          total++;
   ...
   * @return {[type]}          [description]
   */
  function animateToLocation($dom, location, opt, callback) {
   ...
   * @return {[type]}             [description]
   */
  function getIndexByLocation(location, data, locationObj) {
   ...
   * [clearObj 清空对象的属性]
   * @param  {[type]} obj       [description]
   * @param  {[type]} exclusion [排除的键值]
   * @return {[type]}           [description]
   */
  function clearObj(obj, exclusion) {
    for (var i in obj) {
      if (!exclusion || i != exclusion) {
        delete obj[i];
      }
    }
  }
  ...
  /**
   * [switchToDesktopPoint 转化为桌面上的位置]
   * @param  {[type]} $this [description]
   * @param  {[type]} left  [description]
   * @param  {[type]} top   [description]
   * @param  {[type]} opt   [description]
   * @return {[type]}       [description]
   */
  function switchToDesktopPoint($this, left, top, opt) {
    if ($this.hasClass('iconinbox')) {
      var scrollTop = $this.parents('.iconbox-area__container').scrollTop();
      // 如果此时是在盒子里(上方条件)，则需要将盒子里的位置换算成桌面上的位置
      return {
        left: left + opt.currentPageIndex * opt.desktopWidth + opt.openBoxMarginLeft,
        top: top + opt.openBoxMarginTop - scrollTop
      }
    } else {
      return {
        left: left,
  ...
  /**
   * [adjustMoveLocationInBox 微调moveLocation中location与locations的值]
   * @param  {[type]} moveLocation [description]
   * @return {[type]}              [description]
   */
  function adjustMoveLocationInBox(moveLocation) {
    var tempLocationInfo = parseLocationInfo(moveLocation.location);
    if (tempLocationInfo) {
      moveLocation.location = createLocation(-1, tempLocationInfo.row, tempLocationInfo.col);
      for (var i = 0; i < moveLocation.locations.length; i++) {
        var element = moveLocation.locations[i];
        tempLocationInfo = parseLocationInfo(element);
        moveLocation.locations[i] = createLocation(-1, tempLocationInfo.row, tempLocationInfo.col);
      }
    }
  }

  function moveIconToDesktopFromBox($dom, domData, boxData, opt) {
    /**
     * 盒子中的节点移动到桌面上的最后位置
     */
    // 删除盒子里的节点数据
    arrayRemove(boxData.children, domData);
    var locationIconObj = opt.locationBoxObj[boxData.location]
    delete locationIconObj[domData.location];
    // 添加节点数据到桌面最后
    opt.data.push(domData);
    console.log(opt.data)
    delete domData.location;
    var pages = opt.pages;
    // fillLocation(data, opt, override, locations)
    fillLocation(opt.data, opt);
    // 移动节点
    domData.$dom.appendTo($dom.parents('.icondesktop-slidebox'));
    setLocations(domData.$dom, domData, opt, 'set');
    $dom.attr('location', domData.location);
    /**
     * 更新盒子中节点的位置
     */
    fillLocationInBox(boxData.children, locationIconObj, opt, true);
    /**
     * 关闭盒子
     */
    closeIconBox(boxData.$dom, opt);
    /**
     * 调整页数
     */
    var locationInfo = parseLocationInfo(domData.location);
    ...
    domData.$dom.removeClass('iconinbox').addClass('icondesktopbox');
  }

  function arrayIndexOf(data, obj) {
    for (var i = 0; i < data.length; i++) {
      var element = data[i];
      if (element === obj) {
        return i;
  ...
  function arrayRemove(data, obj) {
    var index = arrayIndexOf(data, obj);
    if (index !== -1) {
      data.splice(index, 1);
    }
  }

  function refreshDoms(dataBox, opt, autoFill) {
    var data;
    var locationObj;
    if (dataBox) {
      // 如果是在盒子里(上方条件)
      data = dataBox.children;
      locationObj = opt.locationBoxObj[dataBox.location];
    } else {
      // 如果是在桌面上(上方条件)
      data = opt.data;
      locationObj = opt.locationObj;
    }
    if (!data || !data.length) {
      return;
    }
    if (dataBox) {
      // 如果是在盒子里(上方条件)
      if (autoFill) {
        fillLocationInBox(data, locationObj, opt, true);
        animateDoms(data, opt, -1, function () {
          adjustScrollBar(opt.openBox.$box.find('.iconbox-area__container'));
        });
      } else {
        adjustScrollBar(opt.openBox.$box.find('.iconbox-area__container'));
      ...
      if (autoFill) {
        var pages = opt.pages;
        fillLocation(data, opt, true);
        adjustPageBox(data[0].$dom, pages, opt.pages);
        animateDoms(data, opt, -1);
      }
    }
  }
  ...
  function bindDelBtnClick($delBtn, opt) {
    $delBtn.click(function (e) {
      ...
      var location = $icon.attr('location');
      if ($icon.hasClass('icondesktopbox')) {
        delete opt.locationObj[location];
        $icon.remove();
        refreshDoms(null, opt, opt.desktopAutoFill);
      } else {
        var dataBox = getDataByLocation($icon.parents('.icondesktopbox').attr('location'), opt.data, opt.locationObj);
        delete opt.locationBoxObj[dataBox.location][location];
        $icon.remove();
        refreshDoms(dataBox, opt, opt.desktopAutoFill);
      }
    });
  }
